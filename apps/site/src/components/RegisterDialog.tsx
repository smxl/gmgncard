...
export const RegisterDialog = ({ onClose }: RegisterDialogProps) => {
  const [form, setForm] = useState({ brand: '', email: '', handle: '' });
      ...
      {isSuccess ? (
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <label className="block text-sm text-slate-300">
            品牌/组织
            <input
              className="mt-1 w-full px-4 py-3 rounded-2xl bg-slate-950 border border-slate-800 text-slate-50"
              value={form.brand}
